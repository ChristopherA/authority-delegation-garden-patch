...
AI chat archives (`ai-chats-archive-vault/`) use `type/chat-archive` tags rather than `is_a::[[Chat Log]]`. Chat logs are meeting sidecars; chat archives are standalone conversation records from a different era of the vault.
...
- derived_from::[[Meeting Note]] — chat logs belong to meetings
- relates_to::[[Transcript]] — parallel artifact from the same meeting (speech vs text)
