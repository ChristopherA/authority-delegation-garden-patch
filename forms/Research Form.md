...
Form-typed sections follow their target form type's structural contract. A section marked `is_a::[[Pattern Form]]` should have Context, Forces, Solution, Consequences. A section marked `is_a::[[Model Form]]` should have elements and relationships.
...
1. **Active** — Growing through investigation. New form-typed sections added as findings emerge. `has_status::[[Growing Stage]]`.
...
3. **Depleted** — Most or all form-typed sections have been extracted. The research note retains value as a provenance anchor (where did these nodes come from?) but no longer contains unique content. `has_status::[[Evergreen Stage]]` — the note is complete, not abandoned.
...
- `is_a::[[Research Form]]`
- `has_status::[[Seed Stage]]`, `[[Growing Stage]]`, or `[[Evergreen Stage]]`
- `in_domain::[[Deep Context Architecture]]`
- `source_of::[[Extracted Form]]` — tracks what was extracted from this research
- `extends::[[Inquiry Form]]` — the inquiry that drove this investigation
- `relates_to::[[Citation Form]]` — sources consulted during research
...
- relates_to::[[Opus Form]] — research findings may feed insights presented in opuses
