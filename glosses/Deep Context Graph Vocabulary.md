...
- **`in_precinct::`** — organizational unit (`in_precinct::[[Garden Precinct]]`, `in_precinct::[[Household Precinct]]`). Garden precinct node types carry structural contracts; household precinct types serve operational capture. See [[Precinct as Organizational Unit]].
...
- `renders_as::` — this design node has a runtime rendition at that path or in that format (e.g., persona → agent definition file)
