...
- **Body predicates** that classify it: `is_a::[[Persona Form]]`, `has_status::[[Growing Stage]]`, `in_domain::[[Deep Context Architecture]]`
- **Typed coordination edges**: `coordinates_with::[[Garden Groundskeeper Persona]]`, `coordinated_by::[[Estate Seneschal Persona]]`
- **Principle edges**: `follows::[[Human Authority Over Augmentation Systems]]`
- **Runtime rendition link**: `renders_as::` — connecting design to deployment configuration
...
The estate uses the [[Ephemeral vs Persistent Persona Identity|stateful serverless]] pattern: each agent session reloads identity fresh from version-controlled configuration. No behavioral drift accumulates across sessions because there is no carry-forward of implicit state.
...
The **garden layer** is the canonical design document: a node in the typed graph with predicates, relations, structural sections, and form compliance. It participates in the knowledge graph and serves as the authoritative source for what a persona is and how it relates to everything else.

The **operational layer** is the runtime rendition: a markdown file with YAML frontmatter that the agent harness loads as a system prompt. Tool restrictions, model selection, isolation mode — operational concerns that belong in deployment, not design.
...
**Single-model diversity ceiling.** All estate personas run on the same foundation model. They share the same training distribution, the same conceptual vocabulary, and potentially the same blind spots. The [[The Persona Selection Model|Persona Selection Model]]'s finding about cross-trait inference applies: the estate's functional diversity is genuine but bounded by the model's repertoire.
...
- [[Persona Form]] — structural contract for all estate personas
- [[Estate Seneschal Persona]] — most developed estate-level persona (compound, Growing Stage)
- [[Garden Groundskeeper Persona]] — first compound persona, first external communication
...
- [[Ephemeral vs Persistent Persona Identity]] — the identity model
- [[Behavioral Questions vs Identity Assertions in Persona Design]] — behavioral specification approach
- [[The Persona Spectrum from Role Label to Soul Document]] — classification of specification depth
- [[The Persona Selection Model]] — mechanistic foundation for why persona prompting works
...
- relates_to::[[Ephemeral vs Persistent Persona Identity]]
  - The identity model: ephemeral reset with explicit knowledge persistence.

- relates_to::[[Behavioral Questions vs Identity Assertions in Persona Design]]
  - The specification approach: behavioral decision rules over identity claims.

- relates_to::[[The Persona Spectrum from Role Label to Soul Document]]
