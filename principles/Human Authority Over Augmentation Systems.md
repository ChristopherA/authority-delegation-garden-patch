...
**Session level** — Every session begins by reading state files the human can inspect. Every commit requires human approval. Approval gates surface decisions rather than resolving them silently. Session logs and learning loops make the agent's reasoning visible after the fact.
...
- Chat archive on agency and autonomy in agentic AI systems — augmentation vs substitution
- Estate architecture: rules, processes, and agent configuration files
